...
Fase corrente: BLOCCO A (F8 dark mode + F9 settings) MERGIATO su master (merge commit bcbdb18,
2026-07-23) + audit visivo post-merge chiuso (branch f8-f9-post-merge-audit, commit 2b290ef —
fix --warning-foreground senza variante dark, unico difetto trovato). F0-F6 + F-DEBT completate.
Roadmap F8-F14: spec docs/superpowers/specs/2026-07-21-f8-f14-roadmap-design.md, ADR-0026 →
ADR-0033.
...
corretti: badge secret sempre "configurato" per google_sa_key_path; backup_retention senza
validazione client-side). `alembic heads` = 1 riga (0003), suite **148 test verdi** (144 +4 dal
fix review-finale). **Prossimo passo: via libera al Blocco B (f11-f12-f13-transactions)** —
aprire branch, spec di dettaglio e piano implementativo dedicati prima di scrivere codice, come
già fatto per il Blocco A. Conferma comunque con `git log --oneline master` e
`.superpowers/sdd/progress.md` (ledger completo task-per-task del Blocco A, incluso l'audit
post-merge) se qualcosa risulta diverso da questo elenco.
