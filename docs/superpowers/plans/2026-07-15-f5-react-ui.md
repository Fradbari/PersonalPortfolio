...
**Tech Stack:** Backend: FastAPI + SQLAlchemy (esistenti). Frontend: React (piano originale citava 18; scaffold Vite eseguito in Task 5 ha generato **react@^19.2.7**, default npm al momento dell'esecuzione — non una scelta esplicita, riconciliato post-hoc in DEBT-03/F-DEBT, nessun impatto funzionale osservato) + Vite + TypeScript, TanStack Query, React Router, Tailwind CSS, componenti shadcn/ui (Button, AlertDialog — scritti a mano nel repo, pattern standard shadcn: non è una libreria runtime), Recharts.
